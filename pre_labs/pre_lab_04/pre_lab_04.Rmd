...
**Answer** None appear to be identical, but a lot appear to look only very slightly different with variations as small as periods, commas, or acronyms. There has to be a better way of doing this than to manually go in and do it ourselves. 
...
**Answer** This added a grantee_clean column, in which it refined and standardized columns with similar names. For many of these entries, the biggest change was that the refinr package added all caps to what was originally in the grantee column. Another big change has to do with the "inc" that follows many of these entries, however, this change is not uniform or consistent for all of them. For some it removed the comma before the incorporated, such as with Fair Hill Farm, while for others it removed the incorporated all together, such as with Charles County Arts Alliance, and for others it stayed just the same like with the Domestic Violence Center of Howard County.
...
**Task** Run the following code to group by grantee_clean and Grantee, then filter for recipients that start with W. Name three changes the refinr function made in the first 30 results. 

**Answer** Like before, the refinr function changed the capitalization and punctuation for many of these grantees. It also merged some together that looked similarly as we can see through the count column, which shows some of them with as many as 5 or 7 entries in our data set. 
...
**Answer** Honestly not really, it just takes away the other columns that we aren't currently looking at and replaces them with count.
