...
Copy the url of the graphic you published here. It will begin with <https://datawrapper.dwcdn.net/>. Don't copy the browser's current url. 

**Answer** https://datawrapper.dwcdn.net/MUIYI/1/
...
**Task** Write code to add columns showing the total number of votes for each county and the percentage of votes received by Wes Moore in each county (you did this in Lab 7!), then replace the CSV file in the data folder with it.
...
md_gov_county <- md_gov_county |> 
  clean_names() |>
  mutate(total_votes = cox + moore + lashar + harding + write_ins) |>
...
Describe the changes from the previous map

**Answer** Rather than showing the entire United States, this allowed us to only see Maryland where all of our data dervices from.
...
**Answer** This is so much cleaner, it makes it so when a viewer's cursor hovers over a county it simply (and cleanly) just says the county's name and the percent of votes Wes Moore received. 
...
Put the URL of your map here.

**Answer** https://datawrapper.dwcdn.net/UjbZ0/1/
