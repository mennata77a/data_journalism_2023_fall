...
**Answer**60712 obs. of 8 variables
...
In the space below, write the data type for the "id" column and the "seats" column (it's in light gray, offset by \< \>). **Answer**chr and dbl
...
**Answer**The term shows us the year and the month the starts starts, while the second column shows us how many courses are in each corresponding term. This shows us that typically fall semester has the most courses, while winter courses typically have the least. 
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer**Compared to the previous codeblock, this codeblock is arranged in descending order. This shows our results from the most to the least classes available, rather than showing our results in chronological order by year. The arrange and descending output changed our results the most.
...
**Answer**This codeblock breaks down our dataset even further by arranging our results by department and term. Again, the arrange output changed our results, but this time grouping it with department and term together is what made the most impact on our code.
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer** Compared to the previous codeblock, this one shows us the total number of seats in all of the classes that belong to each term. The output that changed our results the most is the sum function placed inside the summarize function. 
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **Answer**In addition to the sums provided in the previous codeblock, this one also included means and medians  of the number of seats in each term. The output was changed here due to the median and mean functions within the summarize function.
...
**Answer**This codeblock is grouped by department thanks to the group function. It also differs from the previous codeblock by adding a function that codes maximum and minimum seats columns. 
...
1.  Where are the outliers in this dataset? What would the data look like without them?

2.  Which department has grown the most since we have started collecting this data?

3.  What is the standard deviation number of seats?
