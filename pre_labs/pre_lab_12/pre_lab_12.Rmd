...
For this example, we're going to work on loading a simple table of press releases from the Maryland State Department of Education

Let's suppose we can't find a table like that for download, but we do see a version on the website at this URL: <https://news.maryland.gov/msde/category/pressreleases/>.
...
```{r}
mde_url <- "https://news.maryland.gov/msde/category/pressreleases/"
```
...
In our web browser, if we right-click anywhere on the page and select "view source" from the popup menu, we can see the source code. Or you can just copy this into Google Chrome: <view-source:https://news.maryland.gov/msde/category/pressreleases/>.
...
In the environment window at right, look at results Note that it's now a "list of 2".

This gets a little complicated, but what you're seeing here is a nested list that contains two data frames.
...
**Answer** The date column is not dates nor is it a numeric value. The capitalization is also a bit weird.
...
**Answer** I would also add who made or released the press release and what the reason for writing is
...
And there we go. We now have a nice tidy dataframe of Maryland state education press releases.
...
**Answer** I am not completely sure to be honest, but I do know that an <li> tag is typically within an <ul> tag, so I am trying to look at the ids to differentiate them, but I am not comprehending. According to ChatGPT, "from this output alone, it doesn't provide direct information about the number of <ul> (unordered list) tags in the HTML source. To determine the number of <ul> tags, you would need to inspect the broader HTML structure, which is not included in the output you provided."
...
**Task** Run the following code in the gray-colored codeblock below to find the text inside

<li>

tags. Describe the output: 

**Answer** Shows us the text inside each of these li elements, getting rid of all the htmls--makes it easier to tell which are extraneous and which are not.
...
**Answer** Turns it into a dataframe with a column called "value." Shows us which we really don't need like all the ones at the bottom
