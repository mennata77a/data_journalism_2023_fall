...
**Answer** There are 8 columns in this data frame, including ObjectID, Zip Code and Name, Existing, Shape Length and Area as well as geometry
...
**Answer** This is the table which includes the aforementioned columns. It specifically shows Maryland based on zip codes.
...
**Answer** It's a zip code map of Maryland with longitude and latitude coordinates
...
**Answer** I ran the tigris library by installing the package in the console
...
**Answer** Oh my god?? This showed all 3,234 counties in the entire country which is crazy. It shows State codes, GEOID, name, county codes and more.
...
**Answer** This narrowed it down to just Maryland counties
...
**Answer** This creates a Maryland county map using ggplot. It varies from the one above because it closes off the Chesapeake Bay and the Potomac River, which looks super weird but I guess it's actually good because the counties' boundaries extend into the water.
...
**Answer** This looks so insane but it essentially includes the water in our map making, layering the zip code map and the counties map.
...
**Answer** This is a two column dataframe that uses the number of letters of intent to foreclose in each zip code in March 2023. I see it has a MD zipcode from not Maryland though ... weird and sick and twisted in my opinion why can't it just be right.
...
**Answer** This is a data frame made by joining the maryland zip code data and the maryland foreclosure zip data
...
**Answer** This created a zip code map of maryland that showcased that uses shades of black, grey, and blue to show where the most foreclosure notices were.
...
**Answer** I just reloaded the libraries and my census key here
...
**Answer** This pulled out the Maryland county notices based on the population code. This slice thing is so cool and only shows the first row then we pivot longer to get more.
...
**Answer** This pulled out the Maryland county population based on the population code. Geometry = true is the new function here and very important to keep in mind. It contains the "MULTIPOLYGON" data and will draw an outline of each county when we draw a map.
...
**Answer** This made me realize that we're gonna have some trouble with joining because the county names all look different slightly.
...
**Answer** This makes all of our county names look the same, which is so good but deeply infuriating that it didn't come like that. Using str_trim and str_to_title and str_remove_all allowed us to make them all title case, trimmed, and get ride of all the unnecessary white space.
...
**Answer** Now we can join by county! We left join because we want to see everything. Since the number of counties should never change, we will know if we made a mistake.
...
**Answer** This calculates rate of notices per 1000 in the population, which indicates that Charles County has the highest rate. 
...
**Answer** This is a county map of Maryland that indicates the rates of foreclosure notices in each county. Though it is organized with longitude and latitude coordinate axes, it has labels for the majority of these counties--I can't see them too well because of the colors but it does the job.
...
**Answer** This changes the color scale from blues to purples, even though it gets rid of the labels again.
...
**Answer**This changes the color scale once again so the differences are a bit clearer.
