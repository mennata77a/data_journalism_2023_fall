...
**Answer** 4 columns and 3822 rows.
...
**Task** Run the following code to add a month column to our dataframe using lubridate and count the number of calls for each month. In the space below, explain what each step of the code below is doing in English. Note the "label=TRUE" part in the mutate statement. 

**Answer** Here we are creating a new dataframe from the baltco_911_calls data called "baltco_911_calls_by_month". We then created a month column. I actually wasn't super sure what this label=TRUE meant, so I asked ChatGPT the following question: "What does this do when im mutating in the tidyverse? label=TRUE?" I was told that "when you set label = TRUE in functions that support it, like mutate(), it will try to preserve or carry forward the variable labels from the original data frame to the new data frame after the mutation." That actually isn't helpful to me because it's a lot of words that I can't process right now for some reason, but you just said in the pre-lab lecture that it keeps the months labeled by name rather than numbers. (Super helpful, thank you!) We then grouped the data together by the month the 911 calls came in and summarized by the total number of calls to determine how many calls came in each month. We then arranged it from the month that recieved the most number of calls to the month that receieved the least. 
...
**Answer** A blank, grey-ish canvas like thing.
...
**Answer** I see a bar chart with each month on the X axis and the counts of 911 calls on the Y axis.
...
**Answer** This reordered the months on the X axis so that it appears from the month with the least number of calls on the left through the month with the most number of calls on the right.
...
**Answer** This made my bar graph horizontal by flipping teh axes. Meaning it preserved the reordering with the month with the most 911 calls at the top through the month with the least at the bottom. The bars are also seemingly skinnier, but I don't think that's actually an important thing for us.
...
**Answer** This took away that default grey border in the background, which I assume the minimal theme function did that for us.
...
**Answer** This added a blue-ish background, took away the grid in the background and replaced it with singular lines that went through each bar. It did make the titles for the X and Y axes really ugly and positioned too close to the labels in each axes. These are also terrible labels by the way, I hope we fix that soon.
...
**Answer** This added a title to the bar chart, calling it "More 911 Overdose Calls in Warmer Months." It also added a source to our data--thank you, Baltimore County. Also, better labels for each axis finally!
...
**Answer** Back to our data! Here we create a new dataframe from our original one called "baltco_911_calls_by_data," so rather than grouping by months, this code groups our data by full dates. Therefore, rather than 12 rows we now have 366 rows.
...
**Answer** Our data is now presented in a line chart with each point being the day. It looks a little insane, but it definitely gives us the gist of a pattern over the year. Hard to tell specifics though!
...
**Answer** Visually, this didn't change much for me, but from the code I can see that it made it so each tick represented a month rather than a day.  This made the labels on the X axis crazy though.
...
**Answer** This adjusted the angle by 45 degrees, stretching it out somewaht and making the labels on the X-axis signifcantly more legible. Not great, but better!
