...
**Answer** They do not all have the same column structure. Some are lowercase, specifically the Total column, while others are in different orders from one another.
...
county_voters_combined <- 
  bind_rows(list(county_voters_2016, county_voters_2018, county_voters_2020, county_voters_2022))
...
**Answer** I see that while voting republican has remained somewhat consistent throughout the years with a slight increase, voting democratic has steadily increased along with voting for other parties, which has surprisingly grown tremendously since 2016. Similarly, there are way more voters that have been taking to the polls over time.
...
**Answer** There are 24 rows and there should be 24 rows. These two data frames only have one column in common, the county, which is what joins them together.

```{r}
# with nrow included to show row total
maryland_population |> 
  left_join(county_voters_2022, join_by("COUNTY")) |> 
  nrow()
```
...
**Answer** This demographic data includes information on race, geocode, and those over the age of 18.
...
**Answer** The county with the highest 18+ Black population of all population 18 and over is Prince George's County. This county also has the highest percentage of democrats in Maryland with 77% of voters registering that way.
...
**Answer** The county with the highest 18+ Black population of all population 18 and over is Howard County. 52% of this county votes democratic. This does not follow the pattern above in which the racial group lines up with the percentage of democrats in the county. This county, though the greatest percentage of Asian 18 folks+, has a signfinicatly smaller perentage of this racial group than PG County of Black folks.
...
**Answer** This is a <dbl> datatype, meaning it's a number, but it shouldn't be because we're not using it in a calculation or anything--it's just a zip code.
...
**Answer** It is now <chr>, which means it is a character rather than a numeric value.
