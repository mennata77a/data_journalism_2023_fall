...
**Answer** The code created a single column for the total votes across all candidates, not one for each candidate. It also added a column calculating the pct of votes that Wes Moore got.
...
**Answer** I'm going to be completely, I don't exactly what this code is doing here. I think because of the case_when, the mutate statement makes it so each line checks to see if it can detect the state name in the address column. It populates a new column called state with the state's abbreviation.
...
2. For the Maryland expenditure data, which county had the greatest average number of spending by Wes Moore in 2022?
