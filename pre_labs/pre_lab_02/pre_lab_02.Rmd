...
**Answer** We may have a lot of columns in our data set and we only need to work with or see a couple columns, so hiding the extraneous/unnecessary ones may keep us from getting overwhelmed or confused.
...
**Answer:** This introduced mutate, which took the existing expenditure dates column and changed it from a chr column to a date column. The function responsible is mdy (month, date, year), which made this column now a <date> and shown in year, month, day order.
...
**Answer** This function created a month column out of our dates, and then showed each expenditure on the first of each month in 2022. 
...
**Answer** This code created a column for each candidate. It also showcased the total number of votes for each and a decimal of how much of the total votes went to Wes Moore.
...
**Answer** In comparison to the aforementioned code, the arrange function here made it so the data set showed up in descending order, with the greatest percentages of Wes Moore's votes at the very top.
...
**Answer** In comparison to the aforementioned code, the arrange function here made it so the data set showed up in ascending order, with the lowest percentages of Wes Moore's votes at the very top. I don't particualrly find this any more interesting or useful because it doesn't change too much. 
...
**Answer** I looked up "In RStudio, what are some tidyverse functions similar to str_to_upper?" into the Google Search engine and found a few on the refernce page of stringr.tidyverse.org, including:

1. str_to_title(), which converts the title case, meaning only the first letter of each word is capitalized.
2. str_to_sentence(), which converts to sentence case, meaning only the first letter of each sentence is capitalized.
...
**Answer** I'm going to be completely, I don't exactly what this code is doing here. I think because of the case_when, the mutate statement makes it so each line checks to see if it can detect the state name in the address column. It also replaces the state name in the address with the acronym. If it can't find a state name in the address, it labels it as NA.
...
**Answer** At first I did not understand what the NA meant, but after some reading of the explanation about I get it. I think this data makes sense to me, since these seem to be the largest states 
...
1. For the UMD course data, which department offered the most Laws and Ethics courses in 2022?

2. For the Maryland expenditure data, which county had the greatest average number of donations to Wes Moore in 2022?

3. For the UMD course data, which professor offered the most Laws and Ethics courses across departments in 2022?
